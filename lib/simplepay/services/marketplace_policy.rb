  ...
  module Services
    ...
    class MarketplacePolicy < Service
      ...
      def url(sandbox = Simplepay.use_sandbox?)
        sandbox ? self.class.const_get(:COBRANDED_SANDBOX_URL) : self.class.const_get(:COBRANDED_ENDPOINT_URL)
      end
      ...
      def form(attributes = {}, submit = nil)
        ...
        Simplepay::Helpers::FormHelper.content_tag(:form, content, {:method => 'get', :action => url})
      end
      ...
      def set_signature
        ...
        self.signature = Signature.new(URI.parse(url), fields).sign_get if self.respond_to?(:signature=)
      end
